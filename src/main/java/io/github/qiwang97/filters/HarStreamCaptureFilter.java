package io.github.qiwang97.filters;
...
/**
 * fork from {@link net.lightbody.bmp.filters.HarCaptureFilter} but add concurrency support
 */
public class HarStreamCaptureFilter extends HttpsAwareFiltersAdapter {
	private static final Logger log = LoggerFactory.getLogger(HarStreamCaptureFilter.class);
	...
	public volatile LightBrowserProxyServer proxyServer;
	/**
	 ...
	 *                        captured (see {@link CaptureType} for information on data collected for each CaptureType)
	 */
	public HarStreamCaptureFilter(HttpRequest originalRequest, ChannelHandlerContext ctx, Har har, String currentPageRef, Set<CaptureType> dataToCapture, LightBrowserProxyServer server) {
		...
		if (server == null) {
			throw new IllegalStateException("Attempted har capture when proxy server is null");
		}
	...
	public HttpObject serverToProxyResponse(HttpObject httpObject) {
		...
		if (httpObject instanceof LastHttpContent) {
			...
			if (proxyServer.isStopped()) {
				log.warn("proxy is stopped before harEntry send");
			}
		}
	...
	public void serverToProxyResponseReceived() {
		...
		try {
			proxyServer.$entries.onNext(harEntry);
		} catch (Exception e) {
			// just pass error;
			proxyServer.$entries.onError(e);
		}
	}
}
